create_boxplots <- function(data_1, data_2, data_3, data_4){
  ...
  df_new$ens_str <- if_else(df_new$ens < 0, "", "\n ensemble")



  df_new$input_type <- unlist(lapply(1:nrow(df_new), function(x) {strsplit(df_new$name[x], "_")[[1]][1]}))
  df_new$method <- unlist(lapply(1:nrow(df_new), function(x) {strsplit(df_new$name[x], "_")[[1]][2]}))
  ...
  boxplot_plot <- ggplot(df_new, aes(x=name_to_plot, y=value, color=input_type),facets=~Model) +
    geom_boxplot() +
    xlab("") +
    ylab("MAE") +
